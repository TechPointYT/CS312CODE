...
** Student Drawing ** 4 points
 
If DrawingPanel appears to be a little too large, ignore. (There
is an odd behavior that sometimes causes drawing panel to be 10 pixels
too wide and tall in some instances)

-1 if not at least 400 by 200 

-1 if not at least one line

-1 if not at least 1 oval and at least 1 rectangle 

-1 if fewer than 3 colors

** Scintillation Grid ** 6 points

6 points for correct grids
...
Note, style graded on Scintillation Grid only. 
Do not grade style on drawingOne().

-3 does not have a parameterized method to draw a grid at a given location
Lose this if have four different methods, one for each grid to draw.

-2 does not have a parameterized method to draw lines. 
Solution may have one method for all lines or two methods, 
one for horizontal and one for vertical

-2 does not have a parameterized method to draw circles

-2 if any class variables. (static, non-final) instead of passing parameters

-2 for poor style. Okay to take off -1
comments, good identifiers, easy to follow code, consistent spacing and brace style

-2 for disallowed language features, especially if statements and arrays.
casting (int) is okay, return methods are okay

Other style issues:


Comments and Suggestions:
-1 you didn't find the max between 40% more and 2 pixlels more when finding the circle diameter


====================BUILD ERRORS SUBMISSION=====================

====================BUILD ERRORS TESTER=====================

** These errors are likley due to not following the specification for your submisison. **


===============STUDENT OUTPUT FOLLOWS================
IN DrawingPanel Constructor
IN DrawingPanel Constructor
*/
